...
const validate = () => {
  ...
  if (form.value.harga_beli !== "" && Number(form.value.harga_beli) < 0)
    e.harga_beli = "Harga tidak boleh negatif";
  if (form.value.harga_jual !== "" && Number(form.value.harga_jual) < 0)
    e.harga_jual = "Harga tidak boleh negatif";
  errors.value = e;
...
<template>
  <div class="space-y-4">
        ...
        <table class="w-full min-w-900px">
          ...
          <tbody>
            ...
            <tr v-else-if="data.length === 0">
              <td colspan="8">
                <EmptyState message="Belum ada data barang." />
              </td>
            </tr>
            <tr
            ...
            >
              <td class="td">
                {{
                  (pagination.page.value - 1) * pagination.limit.value + i + 1
                }}
              </td>
              ...
              <td class="td text-right">
                <div class="flex items-center justify-end gap-2">
                  <span class="tabular-nums"
                    >{{ item.stok }} {{ item.nama_satuan }}</span
                  >
                  <BaseBadge v-if="isMenipis(item)" type="danger"
                    >Menipis</BaseBadge
                  >
                </div>
              </td>
              <td class="td">
                <div class="flex items-center gap-1">
                  <button
                    class="rounded-lg p-1.5 text-slate-500 transition hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800"
                    title="Detail"
                  ...
                  <button
                    v-if="canEdit"
                    class="rounded-lg p-1.5 text-blue-600 transition hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-900/20"
                    title="Edit"
                    @click="openEdit(item)"
                  >
                  ...
                  <button
                    v-if="canEdit"
                    class="rounded-lg p-1.5 text-red-600 transition hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20"
                    title="Nonaktifkan"
    ...
    >
      <div
        class="mb-4 flex items-center gap-4 rounded-lg bg-slate-50 p-4 dark:bg-slate-800/50"
      >
        <div
          class="flex h-12 w-12 items-center justify-center rounded-lg bg-blue-100 dark:bg-blue-900/40"
        >
      ...
      <dl class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <dd class="text-sm text-slate-900 dark:text-white">
            {{ formatRupiah(selected.harga_beli) }}
          </dd>
        </div>
        <div>
          <dt class="label">Harga Jual</dt>
          <dd class="text-sm text-slate-900 dark:text-white">
            {{ formatRupiah(selected.harga_jual) }}
          </dd>
        </div>
        <div>
          <dt class="label">Stok Saat Ini</dt>
          <dd
            class="flex items-center gap-2 text-sm text-slate-900 dark:text-white"
          >
            <span class="tabular-nums"
              >{{ selected.stok }} {{ selected.nama_satuan }}</span
            >
            <BaseBadge v-if="isMenipis(selected)" type="danger"
              >Menipis</BaseBadge
            >
          </dd>
        </div>
        <div>
          ...
          <dd class="text-sm text-slate-900 dark:text-white">
            {{ selected.min_stok }} {{ selected.nama_satuan }}
          </dd>
        </div>
        <div>
