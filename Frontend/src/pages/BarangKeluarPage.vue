...
import * as service from '../services/barangKeluarService'
import * as barangService from '../services/barangService'
import * as tujuanService from '../services/tujuanService'
...
const form = ref({
  ...
  keterangan: '',
  rows: [{ barang_id: '', qty: 1, harga_jual: '' }],
})

const load = async () => {
  loading.value = true
  try {
    const res = await service.getBarangKeluar({
...
const loadOptions = async () => {
  const [t, b] = await Promise.all([tujuanService.getTujuanAll(), barangService.getBarangAll()])
  tujuans.value = t.data.data
...
const addRow = () => {
  form.value.rows.push({ barang_id: '', qty: 1, harga_jual: '' })
}
...
const onBarangChange = (row) => {
  ...
  if (barang && row.harga_jual === '') {
    row.harga_jual = barang.harga_jual
  }
}
...
const stokCukup = (row) => {
  const barang = findBarang(row.barang_id)
  if (!barang) return true
  return Number(row.qty) <= barang.stok
}
...
const validate = () => {
  ...
  if (!form.value.tujuan_pengeluaran_id) e.tujuan_pengeluaran_id = 'Tujuan pengeluaran wajib dipilih'
  ...
  for (const row of validRows) {
    if (!stokCukup(row)) {
      e.rows = `Stok '${findBarang(row.barang_id)?.nama_barang}' tidak mencukupi`
      break
    }
  }
...
const save = async () => {
  ...
  saving.value = true
  try {
    await service.createBarangKeluar({
      tanggal: form.value.tanggal,
      tujuan_pengeluaran_id: Number(form.value.tujuan_pengeluaran_id),
...
const openDetail = async (item) => {
  ...
  try {
    const res = await service.getBarangKeluarById(item.id)
...
<template>
  <div class="space-y-4">
        ...
        <table class="w-full min-w-[720px]">
          ...
          <tbody>
            ...
            <tr v-else-if="data.length === 0">
              <td colspan="6"><EmptyState message="Belum ada transaksi barang keluar." /></td>
            </tr>
            <tr v-for="(item, i) in data" v-else :key="item.id" class="border-b border-slate-100 last:border-0 dark:border-slate-800">
              ...
              <td class="td">{{ item.nama_tujuan }}</td>
    ...
    <BaseModal v-if="formOpen" title="Transaksi Barang Keluar" width="max-w-3xl" @close="formOpen = false">
      <form class="space-y-4" @submit.prevent="save">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <BaseInput v-model="form.tanggal" label="Tanggal" type="date" :error="errors.tanggal" />
          <BaseSelect v-model="form.tujuan_pengeluaran_id" label="Tujuan Pengeluaran" :options="tujuans" option-value="id" option-label="nama_tujuan" :error="errors.tujuan_pengeluaran_id" />
        </div>
            ...
            <table class="w-full min-w-[640px]">
              <thead>
                <tr class="border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/50">
                  ...
                  <th class="th w-40">Harga Jual</th>
                  <th class="th w-14"></th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(row, i) in form.rows" :key="i" class="border-b border-slate-100 last:border-0 dark:border-slate-800">
                  <td class="td">
                    ...
                    <p v-if="row.barang_id" class="mt-1 text-xs" :class="stokCukup(row) ? 'text-slate-400' : 'text-red-500'">
                      Stok tersedia: {{ findBarang(row.barang_id)?.stok }} {{ findBarang(row.barang_id)?.nama_satuan }}
                      <BaseBadge v-if="!stokCukup(row)" type="danger">Melebihi stok</BaseBadge>
                    </p>
                  </td>
                  <td class="td">
                  ...
                  <td class="td">
                    <input v-model="row.harga_jual" type="number" min="0" class="input" />
                  </td>
                  <td class="td">
      ...
      <template v-if="detailData">
        <div class="mb-4 grid grid-cols-1 gap-3 rounded-lg bg-slate-50 p-4 sm:grid-cols-3 dark:bg-slate-800/50">
          ...
          <div>
            ...
            <dd class="text-sm text-slate-900 dark:text-white">{{ detailData.nama_tujuan }}</dd>
          </div>
          <div>
          ...
          <table class="w-full">
            ...
            <tbody>
              <tr v-for="d in detailData.detail" :key="d.id" class="border-b border-slate-100 last:border-0 dark:border-slate-800">
                ...
                <td class="td text-right font-medium tabular-nums text-slate-900 dark:text-white">{{ formatRupiah(d.qty * d.harga_jual) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
